...
class ExpensesQuerySet(models.QuerySet):
    def serialize(self):
        list_values = list(
            self.values('ExpensesId', 'UserName', 'PersonName', 'Exp_Type_id', 'Exp_Amt', 'Exp_Img', 'Exp_FileName', 'Exp_date',
                        'Exp_comm'))
        print(list_values)
...
class ExpensesEntry(models.Model):
    ...
    def __str__(self):
        return str(self.UserName) + str(self.PersonName) + str(self.Exp_Type_id) + str(self.Exp_Img) + \
